...
export class AchievementsProvider {
    ...
    get(user: IUser, date: Date): IAchievements[] {
        let achievements = this.defaults.TESTACHIEVEMENTS.filter(a => a.user.username === user.username && this.isSameDate(date, a.date));
        return achievements;
    }
